...
1. If given a `*.brief.md`, read it first. Then search `research/` (large sourced notes)
   and `knowledge-base/` (permanent reference) for every note relevant to the topic.
2. Write a synthesis note that follows `knowledge-base/_NOTE_TEMPLATE.md` EXACTLY:
   ...
   - A "What we know" section where every claim carries an inline `[Source: ...]`
     pointing to the specific note it came from.
...
4. Save to `notes/<kebab-slug>.md` (sprint synthesis notes live in notes/, not knowledge-base/).
   Present the summary and the open questions, then suggest proceeding to /write-spec.
